...
import com.cts.hotel.config.KafkaConsumerStream;
import com.cts.hotel.config.KafkaProducerStream;
...
public class HotelInventoryServiceImpl implements HotelInventoryService {
	...
	@Autowired
	private KafkaProducerStream producer;
	
	@Autowired
	private KafkaConsumerStream consumer;
	...
	public Mono<RoomModel> createRoom(RoomModel roomModel) {
		...
		try {
			...
			producer.sendMessages(addRoomTopic, roomModel, count, latch);
...
//		producer.close();
		...
		try {
			Disposable disposable = consumer.consumeMessages(addRoomTopic, latch);
			latch.await(10, TimeUnit.SECONDS);
			disposable.dispose();
